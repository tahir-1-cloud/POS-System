...
var self=this;this._saving=true;this.trigger_up('edition_will_stopped');const destroy=()=>{self.wysiwyg.destroy();self.trigger_up('edition_was_stopped');self.destroy();};if(!this.wysiwyg.isDirty()){return destroy();}
return this.wysiwyg.saveContent(false).then((result)=>{var $wrapwrap=$('#wrapwrap');self.editableFromEditorMenu($wrapwrap).removeClass('o_editable');if(reload){$('body').removeClass('o_connected_user');return self._reload();}else{destroy();}
...
ev.preventDefault();ev.stopPropagation();});this.$translations.each(function(){var $node=$(this);var translation=$node.data('translation');_.each(translation,function(node,attr){var trans=self._getTranlationObject(node);trans.value=(trans.value?trans.value:$node.html()).replace(/[ \t\n\r]+/,' ');$node.attr('data-oe-translation-state',(trans.state||'to_translate'));});});this.$translations.prependEvent('mousedown.translator click.translator mouseup.translator',function(ev){if(ev.ctrlKey){return;}
ev.preventDefault();ev.stopPropagation();if(ev.type!=='mousedown'){return;}
new AttributeTranslateDialog(self,{},ev.target).open();});},_onSave:function(ev){ev.stopPropagation();},});registry.category("website_navbar_widgets").add("TranslatePageMenu",{Widget:TranslatePageMenu,selector:'.o_menu_systray:has([data-action="translate"])',});});;
...
return{trigger:`.oe_overlay.ui-draggable.o_we_overlay_sticky.oe_active .o_handle.${paddingDirection}`,content:Markup(_.str.sprintf(_t("<b>Slide</b> this button to change the %s padding"),direction)),position:position,};}
...
function clickOnSnippet(snippet,position="bottom"){return{trigger:snippet.id?`#wrapwrap .${snippet.id}`:snippet,extra_trigger:"body.editor_enable",content:Markup(_t("<b>Click on a snippet</b> to access its options menu.")),position:position,run:"click",};}
function clickOnSave(position="bottom"){return[{trigger:"button[data-action=save]",in_modal:false,content:Markup(_t("Good job! It's time to <b>Save</b> your work.")),position:position,},{trigger:'body:not(.editor_enable)',auto:true,run:()=>null,}];}
...
/* /theme_loftspace/static/src/js/tour.js defined in bundle 'website.assets_editor' */
odoo.define('@theme_loftspace/js/tour',async function(require){'use strict';let __exports={};const wTourUtils=require('website.tour_utils');const snippets=[{id:'s_cover',name:'Cover',},{id:'s_three_columns',name:'Columns',},{id:'s_title',name:'Title',},{id:'s_images_wall',name:'Images Wall',},{id:'s_call_to_action',name:'Call to Action',},];wTourUtils.registerThemeHomepageTour("loftspace_tour",[wTourUtils.dragNDrop(snippets[0]),wTourUtils.clickOnText(snippets[0],'h1'),wTourUtils.goBackToBlocks(),wTourUtils.dragNDrop(snippets[1]),wTourUtils.dragNDrop(snippets[2]),wTourUtils.dragNDrop(snippets[3]),wTourUtils.dragNDrop(snippets[4]),wTourUtils.clickOnSnippet(snippets[4]),wTourUtils.changeBackgroundColor(),wTourUtils.selectColorPalette(),]);return __exports;});;

/* /website/static/src/js/tours/configurator_tour.js defined in bundle 'website.assets_editor' */
odoo.define("website.configurator_tour",function(require){"use strict";const wTourUtils=require("website.tour_utils");const core=require("web.core");const _t=core._t;let titleSelector='#wrap > section:first-child';let title=$(titleSelector).find('h1, h2').first();if(!title.length){titleSelector=titleSelector.replace('section:first-child','section:nth-child(2)');title=$(titleSelector).find('h1, h2').first();}
let isTitleTextImage=$(titleSelector).hasClass('s_text_image');titleSelector=titleSelector.concat(` ${title.is('h1') ? 'h1' : 'h2'}`);const shapeSelector='#wrap > section[data-oe-shape-data]';const backgroundSelector='#wrap > section:nth-child(2)';const imageStep=isTitleTextImage?wTourUtils.changeImage(titleSelector.replace('h2','img')):wTourUtils.changeBackground();const backgroundColorStep=[wTourUtils.changeBackgroundColor()];if(!isTitleTextImage){backgroundColorStep.unshift(wTourUtils.clickOnSnippet(backgroundSelector));}
const shapeStep=[];if($(shapeSelector).first().length){if(!$(backgroundSelector).is($(shapeSelector))){shapeStep.push(wTourUtils.clickOnSnippet(shapeSelector));}
shapeStep.push(wTourUtils.changeOption('BackgroundShape','we-toggler',_t('Background Shape')));shapeStep.push(wTourUtils.selectNested('we-select-page','BackgroundShape',':not(.o_we_pager_controls',_t('Background Shape')));}
const steps=[wTourUtils.clickOnText(titleSelector),imageStep,...backgroundColorStep,...shapeStep,wTourUtils.changePaddingSize('top'),];wTourUtils.registerThemeHomepageTour('configurator_tour',steps);});